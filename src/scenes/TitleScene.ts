...
export class TitleScene extends Phaser.Scene {
  ...
  private redraw(): void {
    ...
    bigButton(this, positions[0], this.dockY, "icon-garage", COPY.garage, () => this.scene.start("garage"), dockSize);
    bigButton(this, positions[1], this.dockY, "icon-map", COPY.map, () => this.scene.start("map"), dockSize);
    bigButton(this, positions[2], this.dockY, "icon-garage", COPY.vacuums, () => this.scene.start("vacuum-garage"), dockSize);
    bigButton(this, positions[3], this.dockY, "icon-map", COPY.rooms, () => this.scene.start("room-map"), dockSize);
  ...
  private activityCard(x: number, y: number, width: number, height: number, activity: "mow" | "vacuum", onClick: () => void): void {
    ...
    const label = this.add.text(x + width * .18, y - 18, mowing ? "Mow" : COPY.vacuum, {
