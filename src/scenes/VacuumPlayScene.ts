...
import { queueVacuumAsset } from "../systems/AssetCatalog";
...
export class VacuumPlayScene extends Phaser.Scene {
  ...
  private groom!: Phaser.GameObjects.Graphics;
  ...
  private trail: { x: number; y: number; heading: number; floor: FloorType; alpha: number }[] = [];
  private trailAcc = 0;
  ...
  create(): void {
    ...
    this.uiCam = this.cameras.add(0, 0, this.scale.width, this.scale.height, false, "vacuum-ui").setScroll(0,0).setZoom(1);
    ...
    this.groom = this.add.graphics().setDepth(1.2);
    ...
    this.hud = new ActivityHud(this, {
      ...
      finish: () => { this.helperOn = true; this.helperAcc = 0; this.helperRate = Math.max(24, this.debris.remainingCount / 8); },
    }, COPY.cleanFloor);
  ...
  private bindCameras(): void {
    const world: Phaser.GameObjects.GameObject[] = [this.room.floor, ...this.room.props, ...this.debris.views.values(), this.vacuum.sprite, this.vacuum.shadow, this.vacuum.rigLayer, this.cameraFocus, this.targetMarker, this.suctionFx, this.groom];
  ...
  private fitZoom(): void {
    ...
    this.cameras.main.setZoom(composeCamera(v, this.room.width, this.room.height, machineLongAxis).zoom).setBackgroundColor("#3c3029");
  }
  ...
  private drawTarget(): void {
    ...
    const r=12+Math.sin(this.time.now/110)*3; g.lineStyle(3,0x80deea,.9).strokeCircle(t.x,t.y,r); g.lineStyle(2,0xffffff,.55).strokeCircle(t.x,t.y,r+6);
  }
  ...
  private drawSuction(cleaned: number): void {
    ...
    for(let i=0;i<3;i++){const phase=(this.time.now/360+i/3)%1;const sx=ox+c*(22+phase*58)+px*(-30+i*30),sy=oy+s*(22+phase*58)+py*(-30+i*30),mx=ox+c*24+px*(-18+i*18),my=oy+s*24+py*(-18+i*18),ex=ox+px*(-12+i*12),ey=oy+py*(-12+i*12);g.lineStyle(2.5,0xd7f5ff,(1-phase)*(.18+Math.min(.42,cleaned*2)));g.beginPath();g.moveTo(sx,sy);g.lineTo(mx,my);g.lineTo(ex,ey);g.strokePath();}
  }

  private recordGroom(dt: number, floor: FloorType, speed: number): void {
    if(speed<.08)return;this.trailAcc+=dt;if(this.trailAcc<.07)return;this.trailAcc=0;
    this.trail.push({x:this.vacuum.x,y:this.vacuum.y,heading:this.vacuum.heading,floor,alpha:1});
    if(this.trail.length>260)this.trail.splice(0,this.trail.length-260);
  }

  private drawGroom(): void {
    const g=this.groom;g.clear();
    for(const t of this.trail){const carpet=t.floor==="carpet"||t.floor==="rug";const concrete=t.floor==="concrete";const color=carpet?0xe6f1df:concrete?0xc9d0cd:0xffffff;g.lineStyle(carpet?11:6,color,carpet?.18:concrete?.08:.14);const sideX=Math.cos(t.heading+Math.PI/2)*this.vacuum.intakeWidth*.4,sideY=Math.sin(t.heading+Math.PI/2)*this.vacuum.intakeWidth*.4;g.beginPath();g.moveTo(t.x-sideX,t.y-sideY);g.lineTo(t.x+sideX,t.y+sideY);g.strokePath();if(carpet){g.lineStyle(2,0x45644c,.16);g.beginPath();g.moveTo(t.x-sideX*.85,t.y-sideY*.85);g.lineTo(t.x+sideX*.85,t.y+sideY*.85);g.strokePath();}}
  }
  ...
  private showTutorial():void{const w=this.scale.width,h=this.scale.height,steps=[COPY.vacuumTutorial1,COPY.vacuumTutorial2,COPY.vacuumTutorial3];let i=0;const layer=this.add.container(0,0).setDepth(40).setScrollFactor(0);const bg=this.add.rectangle(w/2,h/2,w,h,0x18242a,.48).setInteractive();const msg=labelText(this,w/2,h-168,steps[0],32);let advance=()=>{};const next=bigButton(this,w/2,h-88,"icon-play",COPY.play,()=>advance(),96);advance=()=>{if(++i>=steps.length){save().seenVacuumTutorial=true;persist();layer.destroy();this.tutLayer=undefined;return;}msg.setText(steps[i]);};bg.on("pointerup",advance);layer.add([bg,msg,next]);layer.setData("layout",{bg,msg,next});this.tutLayer=layer;this.cameras.main.ignore(layer);}
