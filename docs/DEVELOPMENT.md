...
public/assets/
  ...
  portraits/<id>.jpg     garage, isolated mint background
...
`scripts/soak.mjs` alternates Mow and Vacuum in one 832×749 browser page for five minutes with Calm Motion off, so wheel/deck/brush, particles, ambience, and transformation effects stay active. Every cycle uses real canvas taps, held CDP touch movement, two-tap Safe Home, scene transitions, heap sampling, and console capture. Duration comes from the page's monotonic clock and scene waits use bounded polling iterations, preventing host sleep or wall-clock corrections from falsifying the result. It writes `.gstack/soak/latest.json`.
...
`npm start` launches `scripts/gateway.mjs`, not the Vite development server. It validates Node and the lockfile, installs locked dependencies only when required, rebuilds only when the source release hash is stale, prefers `0.0.0.0:5173`, serves an app-identified `/healthz`, and opens `/host`. When the default port belongs to another program, family launch scans a bounded range and opens the dashboard on the first available port; an explicitly supplied `PORT` remains strict. A repeated default launch discovers the existing MowerBoy health endpoint and reopens that dashboard without spawning a duplicate. The host page stays readable while setup runs, then shows the computer URL, usable LAN URLs, and a scannable QR code. `PORT`, `MOWERBOY_HOST`, `MOWERBOY_CERT`, and `MOWERBOY_KEY` configure it; certificate and key must be supplied together.
