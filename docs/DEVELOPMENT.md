...
`npm start` launches `scripts/gateway.mjs`, not the Vite development server. It validates Node and the lockfile, installs locked dependencies only when required, rebuilds only when the source release hash is stale, prefers `0.0.0.0:5173`, serves `/healthz`, and opens `/host`. When the default port belongs to another program, family launch scans a bounded range and opens the dashboard on the first available port; an explicitly supplied `PORT` remains strict. The host page stays readable while setup runs, then shows the computer URL, usable LAN URLs, and a scannable QR code. `PORT`, `MOWERBOY_HOST`, `MOWERBOY_CERT`, and `MOWERBOY_KEY` configure it; certificate and key must be supplied together.
