...
| Deterministic verification | Full isolated `npm run verify` passed on commit `d4e002e`: production build, 95 unit checks, gateway smoke, 2 worker tests, 47 browser checks, and exact-offline Mow/Vacuum replay. |
| Repository CI | [Run 33563754737](https://github.com/lukifer23/mowerboy/actions/runs/33563754737) passed commit `d4e002e` on 2026-09-01: build, unit, gateway, and worker-upgrade gates on Ubuntu, Windows, and macOS; cross-OS release inventory consistency; Chrome production flows; and exact offline Mow/Vacuum replay. Soak, visual review, and physical/human acceptance remain separate gates. |
