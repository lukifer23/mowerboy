...
| Deterministic verification | Full `npm run verify` passed on the preceding 2026-09-01 app change set. The current service-worker and Finish-timing corrections separately pass production build, 95 unit checks, 2 worker tests, repeated exact-offline Mow/Vacuum replay, and the focused both-activity Pause/Quiet/Finish browser contract; complete local and remote gates must pass on the committed fix. |
...
| Production build | Vite 8.2.2 build passes; 59 canonical production assets validate at 15.46 MiB; release `40b871678c419c3e` contains 67 files at 16.79 MiB |
...
| Windows shortcut | Desktop `.lnk` targets hidden `wscript.exe` → `scripts/windows-launch.vbs` with the project working directory and custom icon. A 2026-09-01 current-source launch reached release `40b871678c419c3e` on documented fallback port 5176 because earlier verified gateways occupied 5173–5175; dashboard, `/host/qr.svg`, and game returned 200, and both detected LAN URLs used the same port. A repeat launch reused the same sole current-source gateway rather than creating another. |
